...
const Consumable = ({ grp, idx, itemData }) => {
  ...
  return (
    <div className="">
      <div className=" md:block ">
        {/* nav start */}
        <div className="navbar bg-[#EBEBEB] h-1 my-4">
          ...
          <div className="flex justify-center items-center ">
            <div className="navbar-end flex justify-center">
              ...
              <Link to={`/category/${idx}`}>
                <div className="flex text-[16px] font-bold justify-center border-b-2 border-[#F26734] items-center text-[#F26734] gap-5 pl-3">
                  All <FaChevronRight />
                </div>
              </Link>
            </div>
                ...
                <Link to={`../item/${item.item_code}`} key={idx}>
                  <div className="border-2 flex flex-col items-center justify-center h-[210px] bg-white">
                    ...
                    <p className="font-bold text-sm text-center">
                      {item.web_item_name.length > 14
                        ? `${item.web_item_name.substring(0, 14)}...`
                        : item.web_item_name}
                    </p>
                    <p className="flex justify-center items-center gap-1 pl-2">
                      <FaBangladeshiTakaSign />
                      <p className="font-semibold">{rate(item?.item_code)}</p>
                    </p>
                  </div>
                </Link>
              ))}
